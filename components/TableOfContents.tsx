...
const TableOfContents: React.FC<TableOfContentsProps> = ({
        ...
        {items.map((chapter) => {
          ...
          return (
            <button
              ...
              className="w-full text-left py-3 px-3 flex items-center gap-4 transition-all group hover:bg-amber-900/5 rounded-md border border-transparent hover:border-amber-900/10"
            >
              <div className="w-8 h-8 rounded-full border border-amber-900/20 flex items-center justify-center bg-[#fdfaf1] group-hover:scale-110 transition-transform shadow-sm">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-lg font-book font-medium text-amber-950 group-hover:text-amber-800 transition-colors truncate">
                  {chapter.title}
                </h3>
              </div>

              {bookmarks.includes(chapter.id) && (
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" className="text-amber-600 flex-shrink-0"><path d="m19 21-7-4-7 4V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16z" /></svg>
              )}
            </button>
          );
        })}
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-[#fdfaf1]">
      ...
      <div className="h-full flex flex-col max-w-2xl mx-auto relative z-10">

        {/* Header Section */}
        <div className="pt-8 pb-4 px-6 text-center">
          <div className="w-16 h-1 mx-auto bg-amber-900/20 rounded-full mb-6"></div>
          <h2 className="text-4xl md:text-5xl font-bold font-cursive bg-gradient-to-b from-amber-700 to-amber-900 bg-clip-text text-transparent drop-shadow-sm mb-2 leading-relaxed py-2">{t.index}</h2>
